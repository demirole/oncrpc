 ...
 */
void
xdrrec_create(XDR *xdrs, u_int sendsize, u_int recvsize, caddr_t tcp_handle, xdrrec_tcp_fn_t readit, xdrrec_tcp_fn_t writeit)
{
	RECSTREAM *rstrm =
	...
	for (rstrm->out_base = rstrm->the_buffer;
		(u_longlong)rstrm->out_base % BYTES_PER_XDR_UNIT != 0;
		rstrm->out_base++);
...
xdrrec_getlong(XDR *xdrs, long *lp)
{
	...
	if ((rstrm->fbtbc >= sizeof(long)) &&
		(((s_longlong)rstrm->in_boundry - (s_longlong)buflp) >= sizeof(long))) {
...
xdrrec_putbytes(XDR *xdrs, caddr_t addr, u_int len)
{
	...
	while (len > 0) {
		current = (int)((u_longlong)rstrm->out_boundry - (u_longlong)rstrm->out_finger);
		current = (len < current) ? len : current;
...
xdrrec_endofrecord(XDR *xdrs, bool_t sendnow)
{
	...
	if (sendnow || rstrm->frag_sent ||
		((u_longlong)rstrm->out_finger + sizeof(u_longlong) >=
		(u_longlong)rstrm->out_boundry)) {
		rstrm->frag_sent = FALSE;
		return (flush_out(rstrm, TRUE));
	}
	len = (u_long)((u_longlong)(rstrm->out_finger) - (u_longlong)(rstrm->frag_header)) -
	   sizeof(u_long);
...
flush_out(RECSTREAM *rstrm, bool_t eor)
{
	...
	u_long len = (u_long)((u_longlong)(rstrm->out_finger) -
		(u_longlong)(rstrm->frag_header)) - sizeof(u_long);

	*(rstrm->frag_header) = htonl(len | eormask);
	len = (u_long)((u_longlong)(rstrm->out_finger) - (u_longlong)(rstrm->out_base));
...
fill_input_buf(RECSTREAM *rstrm)
{
	...
	where = rstrm->in_base;
	i = (int)((u_longlong)rstrm->in_boundry % BYTES_PER_XDR_UNIT);
	where += i;
	len = rstrm->in_size - i;
...
get_input_bytes(RECSTREAM *rstrm, caddr_t addr, int len)
{
	...
	while (len > 0) {
		current = (int)((s_longlong)rstrm->in_boundry - (s_longlong)rstrm->in_finger);
		if (current == 0) {
